...
public class JwtUtil {
    ...
    public Claims extractClaims(String token) {
        return Jwts.parserBuilder()
                .setSigningKey(getSecretKey()) // Define a chave secreta para validar a assinatura do token
                .build()
                .parseClaimsJws(token) // Analisa o token JWT e obtém as claims
                .getBody(); // Obtém o payload (corpo) do token, que cont'm as claims
    }

    // Extrai o nome de usuário do token JWT
    public String extrairCpfToken(String token) {
    ...
    public boolean validateToken(String token, String username) {
        ...
        final String extractedUsername = extrairCpfToken(token);
